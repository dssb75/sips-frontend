...
import { Composicion, createComposicion, deleteComposicion, listComposiciones, updateComposicion } from "@/services/calendar";
...
export default function ComposicionPage() {
  const [items, setItems] = useState<Composicion[]>([]);
  ...
  const [createForm, setCreateForm] = useState<UpsertFormState>({ nombre: "" });
  const [editingItem, setEditingItem] = useState<Composicion | null>(null);
  const [editForm, setEditForm] = useState<UpsertFormState>({ nombre: "" });
  const [detailItem, setDetailItem] = useState<Composicion | null>(null);
  const [deletingItem, setDeletingItem] = useState<Composicion | null>(null);
  ...
  const loadItems = async () => {
    ...
    setError(null);
    try {
      const response = await listComposiciones();
      setItems(response);
      setSelectedIds(new Set());
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : "No se pudo cargar el listado de composiciones.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const hasOpenModal = showCreateModal || editingItem !== null || detailItem !== null || deletingItem !== null;
  ...
  const handleCreateSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setInfo(null);
    try {
      await createComposicion({ nombre });
      setInfo("Composicion creada.");
      closeCreateModal();
      await loadItems();
      setCurrentPage(totalPages);
    } catch (createError) {
      setError(createError instanceof Error ? createError.message : "No se pudo crear la composicion.");
    } finally {
      setCreating(false);
    }
  };

  const startUpdate = (item: Composicion) => {
    setEditingItem(item);
  ...
  const handleUpdateSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setInfo(null);
    try {
      await updateComposicion(editingItem.id, { nombre });
      setInfo("Composicion actualizada.");
      cancelUpdate();
      await loadItems();
    } catch (updateError) {
      setError(updateError instanceof Error ? updateError.message : "No se pudo actualizar la composicion.");
    } finally {
      setUpdatingId(null);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setInfo(null);
    try {
      await deleteComposicion(deletingItem.id);
      setInfo("Composicion eliminada.");
      setDeletingItem(null);
      await loadItems();
    } catch (deleteError) {
      setError(deleteError instanceof Error ? deleteError.message : "No se pudo eliminar la composicion.");
    } finally {
      setDeletingId(null);
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (deleted === ids.length) {
      setInfo(`Se eliminaron ${deleted} composicion(es).`);
    } else {
      setError(`Se eliminaron ${deleted} de ${ids.length} composiciones seleccionadas.`);
    }
    ...
    <section className="space-y-6">
      <header className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Modulo de composicion</p>
            <h1 className="mt-2 text-2xl font-bold text-slate-800">Listado de composiciones</h1>
            <p className="mt-2 text-sm text-slate-500">Administra exclusivamente los nombres de composicion.</p>
          </div>
      ...
      {showCreateModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" role="presentation">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-xl" role="dialog" aria-modal="true" aria-label="Agregar composicion">
            <h2 className="text-lg font-semibold text-slate-800">Agregar composicion</h2>
      ...
      {editingItem ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" role="presentation">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-xl" role="dialog" aria-modal="true" aria-label="Actualizar composicion">
            <h2 className="text-lg font-semibold text-slate-800">Actualizar composicion</h2>
      ...
      {detailItem ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" role="presentation">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl" role="dialog" aria-modal="true" aria-label="Detalle composicion">
            <h2 className="text-lg font-semibold text-slate-800">Detalle composicion</h2>
            <div className="mt-4 space-y-2 text-sm text-slate-700">
              <p><span className="font-semibold">Nombre:</span> {detailItem.nombre}</p>
            </div>
            <button
              type="button"
      ...
      {deletingItem ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" role="presentation">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl" role="dialog" aria-modal="true" aria-label="Eliminar composicion">
            <h2 className="text-lg font-semibold text-slate-800">Eliminar composicion</h2>
            <p className="mt-3 text-sm text-slate-600">
              Se eliminara la composicion {deletingItem.nombre}. Esta accion no se puede deshacer.
            </p>
